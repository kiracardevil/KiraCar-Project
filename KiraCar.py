...
# --- CONFIG (ใช้ URL แบบคงที่เพื่อหยุดการ Loop) ---
...
@st.cache_data(ttl=300) # จำข้อมูลไว้ 5 นาที เพื่อความเสถียรของแอป
def load_data():
    try:
...
# --- 2. อัปเดตสถานะรถ (ฟีเจอร์ที่คุณต้องการใช้) ---
elif menu == "🔄 อัปเดตสถานะรถ":
    st.title("🔄 เปลี่ยนสถานะ / บันทึกการขาย")
    if not df.empty:
        # สร้างตัวเลือกรายชื่อรถ
        ...
        # ดึง ID ของรถคันที่เลือกมาประมวลผล
        ...
        with st.form("update_car_status"):
            ...
            if st.form_submit_button("✅ บันทึกการเปลี่ยนแปลง"):
                # คำนวณต้นทุนรวมและกำไรใหม่ทันที
...
# --- 3. บันทึกรถเข้าใหม่ ---
    ...
    with st.form("add_new_car"):
        ...
        if st.form_submit_button("🚀 บันทึกเข้าระบบ"):
            total = buy + fix
            profit = sell - total
            # ส่งข้อมูล 12 คอลัมน์ตามลำดับ (A-L)
